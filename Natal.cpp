...
void main()
{
	...
	string produto[6] = { "Batatas Fritas", "Bolo Rei", "Perfume", "Par de Meias", "Pijama", "Chocolate com Ginja" };
	int tabela[100][100], total = 0, totalP = 0, totalT = 0;
	for (int i = 0; i < 22; i++)
	{
		for (int j = 0; j < 6; j++)
		{
			cout << "quantas/os " << produto[j] << " a/o " << pessoas[i] << " trouxe para o natal ";
			cin >> tabela[j][i];
		}
	}
	for (int i = 0; i < 22; i++)
	...
	cout << "O total de produtos trazidos foram " << totalP << "\n";
	for (int i = 0; i < 6; i++)
	{
		totalT = 0;
		for (int j = 0; j < 22; j++)
		...
		cout << "No total tem " << totalT << " " << produto[i] << "\n";
	}